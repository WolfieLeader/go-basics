...
const (
	maxInt64  int64  = (1 << 32)     // Maximum for int64 (since it has positive and negative)
	minInt64  int64  = -(1 << 32)    // Minimum for int64 (since it has positive and negative)
	maxUint64 uint64 = (1 << 64) - 1 // Maximum value for uint64 (only positive values)
	minUint64 uint64 = 0             // Minimum value for uint64
)

func main() {
	// Short declaration (inside a function only)
	name := "John Doe"
	age := 25
	isEmployed := true
